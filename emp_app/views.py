...
def removeEmp(request, empID=None):
    if request.method == 'POST':
        emp_id = request.POST.get('emp_id') or empID

        if emp_id:
            try:
                emp = Employee.objects.get(emp_id=emp_id)
                if empID:  # If empID is provided, delete the employee
                    emp.delete()
                    messages.success(request, "Employee removed successfully.")
                    return redirect("/")
                else:  # If empID is not provided, return employee data for confirmation
                    response = json.dumps({
                        'status': 'success',
                        'empID': emp.emp_id,
